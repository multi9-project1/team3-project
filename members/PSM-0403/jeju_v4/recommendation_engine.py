...
class RecommendationEngine:
    ...
    def _classify_reviews(self, reviews_text: str):
        """GPT로 리뷰를 긍정/부정 최대 3개씩 분류. 실패 시 키워드 기반 폴백."""
        import json, re
        reviews = [
        ...
        if self.ai:
            try:
                sample = reviews[:15]
                prompt = (
                    f"다음 한국어 리뷰 목록을 긍정과 부정으로 분류해줘.\n"
                    f"리뷰: {json.dumps(sample, ensure_ascii=False)}\n"
                    f"JSON만 반환. 형식: {{\"pos\": [\"...\"], \"neg\": [\"...\"]}}\n"
                    f"각 최대 3개씩, 원문 그대로. 코드블록 없이 순수 JSON만."
                )
