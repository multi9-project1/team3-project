...
class KakaoService:
    ...
    def search_accommodation(self, name: str) -> List[Dict]:
        ...
        # 띄어쓰기와 브랜드 표기 편차를 흡수하도록 검색어 후보를 확장한다.
        query_candidates = self._build_search_queries(keyword)

        # 제주 반경 검색 결과를 먼저 모아 제주 지역 결과를 우선 노출한다.
        merged_results: List[Dict] = []
        for query in query_candidates:
            merged_results.extend(self._keyword_search(query, size=7, use_jeju_bias=True))

        # 도로명·지번 주소 입력도 바로 찾을 수 있도록 주소 검색을 함께 수행한다.
        merged_results.extend(self._address_search(keyword, size=5))
        unique_results = self._dedupe_results(merged_results)

        # 제주 우선 결과가 부족하면 전국 검색으로 보완해 검색 누락을 줄인다.
        if len(unique_results) < 3:
            nationwide_results: List[Dict] = []
            for query in query_candidates:
                nationwide_results.extend(self._keyword_search(query, size=7, use_jeju_bias=False))
            unique_results = self._dedupe_results(unique_results + nationwide_results)

        return unique_results[:8]

    # 제주 우선 검색과 전국 검색에 공통으로 사용할 검색어 후보를 정리한다.
    ...
    def _keyword_search(self, query: str, size: int = 5,
    ...
    # 주소 검색 결과를 keyword 검색 결과와 같은 형태로 맞춰 UI에서 바로 재사용한다.
    ...
    # 키워드 검색과 주소 검색 결과를 합칠 때 같은 장소가 반복 노출되지 않도록 중복 제거한다.
