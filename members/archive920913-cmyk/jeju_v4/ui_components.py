...
def render_place_card(slot: Dict, place: Dict, reason: str,
    ...
    with st.container():
        ...
        # ── 대표 리뷰 (📊 CSV) ──
        raw_reviews = str(place.get("reviews_text", ""))
        if raw_reviews and raw_reviews != "nan":
            reviews = [r.strip() for r in raw_reviews.split("|") if len(r.strip()) > 10][:3]
            if reviews:
                with st.expander("💬 대표 리뷰 3개  (📊 CSV 데이터에서 추출)", expanded=False):
                    for rv in reviews:
